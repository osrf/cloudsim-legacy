...
def check_version(team, version, user):
    '''
    ...
    cmd = ('ssh ' + DEFAULT_SSH_OPTS + ' -i ' + key + ' ' + user + '@' + ip + ' cat /var/www/VERSION')

    po = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = po.communicate()
    if po.returncode == 0:
        if out.rstrip() != version:
            print 'Incorrect version in %s' % constellation


def go(yaml_file, version, one_team_only, user):
    '''
        ...
        with open(yaml_file) as infof:
            ...
            for team in info['teams']:
                if ((one_team_only and team['team'] == one_team_only) or
                   not one_team_only):
                    Thread(target=check_version,
                           args=[team, version, user]).start()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=('Checks if all the CloudSim Jrs have the correct version'))

    parser.add_argument('yaml_file', help='YAML file with the team and tasks info')
    parser.add_argument('version', help='Expected CloudSim version')
    ...
    go(arg_yaml_file, arg_version, arg_team, arg_user)
