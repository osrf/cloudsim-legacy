...
sudo cp $DIR/distfiles/apache2/* /etc/apache2/sites-available

echo
echo disable all websites
echo
...
echo Copy dev files to website directory

sudo cp -a $DIR/* /var/www/
sudo find /var/www -name "*.pyc" -exec rm {} \;


echo Change owner of CloudSim files for security: 
...
echo Setup a daemons to launch and monitor simulations for us
...
echo Restarting the web server daemon
...
if [ "$basic_auth" == "0" ]; then
  echo ""
  echo "*******************************************************************"
  echo "Configured to use OpenID with Google emails."
  echo "*******************************************************************"
fi
