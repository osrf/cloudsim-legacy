...
if not matches:
    ...
    machine = matches[0]
    reboot, reboot_err = machine.reboot()
    if reboot:
        print("Machine rebooted.  Proceed to the <a href=\"/cloudsim/inside/cgi-bin/console.py\">Console</a>.")
    else:
        print("Error while rebooting: %s"%(reboot_err))
