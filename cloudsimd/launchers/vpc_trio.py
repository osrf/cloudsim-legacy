...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i /home/ubuntu/cloudsim/%s.pem ubuntu@%s 'source /opt/ros/fuerte/setup.sh && source /usr/share/gazebo/setup.sh && gzlog stop && sleep 5 && while [ "`timeout 1 gzstats -p 2>/dev/null |cut -d , -f 4 | tail -n 1`" != " F" ]; do sleep 1; done && killall -INT roslaunch'
