...
from launch_utils.startup_scripts import create_openvpn_client_cfg_file,\
    ...
    get_simulator_deploy_script, get_puppet_install_script
...
from launch_utils.provision import install_requirements, provision, wait_for_ssh


#########################

from launch_utils.testing import get_test_path, get_boto_path
...
def log(msg, channel=__name__, severity="info"):
    """
    Calls the log method, but using the current module name as the 
    message source
    """
...
def deploy_constellation(constellation_name, cloud_provider, machines,
    ...
    ssh_client.cmd('apt-get -y install openvpn')
...
def launch(constellation_name, tags):
    ...
    constellation = ConstellationState(constellation_name)
    use_non_stable = \
        constellation.get_value('configuration') == 'Sim-x'
    ...
    if use_non_stable:
        simulator_image_key = 'ubuntu_1204_x64'
    else:
        simulator_image_key = 'ubuntu_1204_x64_simulator'

    ip = "127.0.0.1"
    machines = {}
    machines['sim'] = {'hardware': 'g2.2xlarge',  # 'cg1.4xlarge'
    ...
    if use_non_stable:
        ubuntu_sources_repo = get_aws_ubuntu_sources_repo(
                                                  credentials_fname)
        script = get_puppet_install_script(ubuntu_sources_repo, "sim")
    else:
        script = """#!/bin/bash
# Exit on error
set -ex
...
# basically do nothing and leave a trace
echo `date`
"""

    ip, aws_id, key_pair_name = acquire_aws_single_server(
                              constellation_name,
                              ...
                              tags=tags)
    log("machine %s created, ip %s" % (aws_id, ip))

    ssh_key_fname = os.path.join(constellation_directory,
                                         "%s.pem" % key_pair_name)
    wait_for_ssh(count=100, sleep=2, ip=ip, ssh_key_fname=ssh_key_fname)
    # install_requirements(ip, ssh_key_fname)
...
class TestSimX(unittest.TestCase):
    """
    ...
    def xtest_sim(self):
        ...
        gpu_driver_list = ['nvidia-current',
                           ...
                           'nvidia-current-dev',
                           'nvidia-cg-toolkit']
        s = get_simulator_script(ubuntu_sources_repo,
                                 drcsim_package_name,
                                 ip,
                                 ros_master_ip=ip,
    ...
    def test_launch(self):
        ...
        data['configuration'] = 'Sim-x'
        data['username'] = 'tester'

        config = {}
        config['machines_directory'] = get_test_path("simtest")
