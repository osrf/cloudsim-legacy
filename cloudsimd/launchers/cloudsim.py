...
def monitor(username, constellation_name, credentials_ec2, counter):
    ...
    try:
        ...
        # launch_db.log("constellation %s state %s" % (constellation_name, constellation_state) )
        if constellation_state == "terminated":
            launch_db.log("constellation_state terminated for  %s " % constellation_name)
            return True
    except:
        launch_db.log("Can't access constellation  %s data" % constellation_name)
        return True
    ...
    if sim_state_index >= machine_states.index('packages_setup'):
        ...
        if o == 0:
            ...
            launch_db.log('ping simulator %s %s %s %s' % (mini, avg, maxi, mdev))
            launch_events.latency_event(username, CONFIGURATION, constellation_name, sim_machine_name, mini, avg, maxi, mdev)

        if sim_state_index >= machine_states.index('running'):
            try:
                ping_gl = ssh_sim.cmd("bash cloudsim/ping_gl.bash")
                launch_db.log("cloudsim/ping_gl.bash = %s" % ping_gl)
                launch_events.gl_event(username, CONFIGURATION, constellation_name, sim_machine_name, "red", "running")

            except Exception, e:
                launch_db.log("monitor: cloudsim/ping_gl.bash error %s" % e)
                launch_events.gl_event(username, CONFIGURATION, constellation_name, sim_machine_name, "red", "Not running")

            try:
                ping_gazebo = ssh_sim.cmd("bash cloudsim/ping_gazebo.bash")
                launch_db.log("cloudsim/ping_gazebo.bash = %s" % ping_gazebo)
                launch_events.simulator_event(username, CONFIGURATION, constellation_name, sim_machine_name, ping_gazebo)
            except Exception, e:
...
def launch(username, constellation_name, tags, credentials_ec2, constellation_directory, website_distribution=CLOUDSIM_ZIP_PATH):
    ...
    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "orange", "starting")
    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "yellow", "starting")

#    launch_events.launch_event(username, CONFIGURATION, constellation_name, router_machine_name, "yellow", "acquiring public ip")
...
#    sshclient.clean_local_ssh_key_entry(router_ip)
    ...
    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "orange", "setting up security groups")
    ...
    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "yellow", "creating ssh keys")
    ...
    try:
        launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "orange", "booting")
        ...
        launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "red", "%s" % e)
        raise
        ...
        for r in ec2conn.get_all_instances():
            ...
            if r.id == res.id:
                ...
                if state == 'running':
                    ...
                    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, color, 'network_setup')
                    done = True
                launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, color, state)
                if color == "yellow":
                    color = "orange"
                else:
                    color = "yellow"
    ...
    ssh_sim = sshclient.SshClient(sim_machine_dir, sim_key_pair_name, 'ubuntu', sim_ip)

    networking_done = task_list.get_ssh_cmd_generator(ssh_sim, "ls launch_stdout_stderr.log", "launch_stdout_stderr.log", constellation, "sim_state", 'packages_setup', max_retries=1000)
    #task_list.empty_ssh_queue([networking_done], sleep=2)

    color = "orange"
    for g in networking_done:
        time.sleep(2)
        launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, color, "waiting for setup done")
        if color == "yellow":
            color = "orange"
        else:
            color = "yellow"

    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "orange", "creating monitoring scripts")
    find_file_sim = """
    #!/bin/bash
    ...
    launch_events.launch_event(username, CONFIGURATION, constellation_name, sim_machine_name, "orange", "creating zip file bundle")
    ...
    sim_setup_done = task_list.get_ssh_cmd_generator(ssh_sim, "ls cloudsim/setup/done", "cloudsim/setup/done", constellation, "sim_state", 'running', max_retries=100)
    ...
    launch_db.log("uploading '%s' to the server to '%s'" % (website_distribution, remote_fname))

    out = ssh_sim.upload_file(website_distribution, remote_fname)
    launch_db.log(" upload: %s" % out)
    upload_done = task_list.get_ssh_cmd_generator(ssh_sim, "ls cloudsim/setup/done", "cloudsim/setup/done", constellation, "sim_state", 'running', max_retries=100)
    ...
    launch_db.log("uploading '%s' to the server to '%s'" % (fname_zip, remote_fname))
    ...
    launch_db.log("uploading '%s' to the server to '%s'" % (credentials_ec2, remote_fname))
...
def terminate(username, constellation_name, credentials_ec2, constellation_directory):
    ...
    launch_events.launch_event(username, CONFIGURATION, constellation_name, resources['sim_machine_name'], "orange", "terminating")

    ec2conn = aws_connect(credentials_ec2)[0]
    constellation = launch_db.ConstellationState(username, constellation_name)
    constellation.set_value('constellation_state', 'terminating')

    launch_db.log("terminate %s [user=%s, constellation_name=%s" % (CONFIGURATION, username, constellation_name))
    ...
    try:
        ...
        launch_db.log("error cleaning up simulation key %s: %s" % (sim_key_pair_name, e))

    try:
        security_group_id = resources['sim_security_group_id']
        ec2conn.delete_security_group(group_id=security_group_id)
    except Exception, e:
        launch_db.log("error cleaning up sim security group %s: %s" % (security_group_id, e))
...
class DbCase(unittest.TestCase):
    ...
    def test_set_get(self):
        ...
        value = {'a': 1, 'b': 2}
        expiration = 25
        launch_db.set_constellation_data(user_or_domain, constellation, value, expiration)
